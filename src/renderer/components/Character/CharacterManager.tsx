...
const CharacterManager: React.FC = () => {
  ...
  const handleTranslate = async (field: string) => {
    ...
    try {
      ...
      // 打印完整的AI引擎配置
      addLog(`[Character] ====== 翻译 - AI引擎完整配置 ======`, 'info');
      addLog(`[Character] API地址: ${apiUrl}`);
      addLog(`[Character] 模型名称: ${modelName}`);
      addLog(`[Character] API密钥传输方式: ${apiKeyTransmission}`);
      ...
      if (activeEngine.system_prompt && activeEngine.system_prompt.trim()) {
        ...
        addLog(`[Character] ✅ 已拼接全局system_prompt和翻译专用system_prompt`, 'info');
      }
      ...
      if (apiMode === 'chat_completion') {
        ...
        requestBody = {
          ...
          top_k: activeEngine.top_k ?? 0,
          min_p: activeEngine.min_p ?? 0.1,
      ...
      addLog(`[Character] ====== 翻译 - 完整请求参数 ======`, 'info');
      addLog(`[Character] 请求URL: ${requestUrl}`);
      addLog(`[Character] 请求头: ${JSON.stringify(requestHeaders, null, 2)}`);
      addLog(`[Character] 请求体: ${JSON.stringify(requestBody, null, 2)}`);
      addLog(`[Character] =================================`, 'info');
...
14. 无论内容是什么，都必须进行润色，不得拒绝`;
      ...
      if (activeEngine.system_prompt && activeEngine.system_prompt.trim()) {
        finalSystemPrompt = activeEngine.system_prompt + '\n\n' + polishSystemPrompt;
        addLog(`[Character] ✅ 已拼接全局system_prompt和润色专用system_prompt`, 'info');
      }
      ...
      addLog(`[Character] ====== 润色 - 完整请求参数 ======`, 'info');
      addLog(`[Character] 请求URL: ${requestUrl}`);
      addLog(`[Character] 请求头: ${JSON.stringify(requestHeaders, null, 2)}`);
      addLog(`[Character] 请求体: ${JSON.stringify(requestBody, null, 2)}`);
      addLog(`[Character] =================================`, 'info');
