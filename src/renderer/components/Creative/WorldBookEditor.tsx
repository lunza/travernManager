...
const WorldBookEditor: React.FC<WorldBookEditorProps> = ({ worldbookId }) => {
  const {
    creatives,
    currentCreativeId,
    updateWorldBook,
  ...
  const worldBook = currentCreative?.worldBooks.find(wb => wb.id === worldbookId);
  ...
  // 保存世界书内容
  const handleSave = () => {
    if (currentCreativeId && worldbookId) {
      updateWorldBook(currentCreativeId, worldbookId, { content: editingContent });
      addWorldBookVersion(currentCreativeId, worldbookId, editingContent, '手动保存');
  ...
  const handleSaveGeneratedContent = () => {
    ...
    if (contentToSave && currentCreativeId) {
      ...
      updateWorldBook(currentCreativeId, worldbookId, { content: contentToSave });
      addWorldBookVersion(currentCreativeId, worldbookId, contentToSave, 'AI生成');
  ...
  const handleGenerate = async () => {
    ...
    const handleStreamComplete = (data: any) => {
      if (data.data) {
        ...
        updateWorldBook(currentCreativeId, worldbookId, { content: generated });
        addWorldBookVersion(currentCreativeId, worldbookId, generated, 'AI生成');
      ...
      children: currentCreativeId ? (
        <CreativeOptimize
          ...
          onContentChange={(content) => {
            ...
            if (currentCreativeId) {
              updateWorldBook(currentCreativeId, worldbookId, { content });
            }
          }}
